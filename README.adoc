...
This fork carries documentation only: no Spring Boot source, no Gradle build.
It publishes the reference documentation in two languages, English at the site root and Vietnamese under `/vi/`.

* `upstream/` is a read-only mirror of the upstream documentation, refreshed by `scripts/sync-spring-boot-docs.sh` and pinned by `.upstream-version`. Do not edit it by hand.
* `vi/` holds the Vietnamese translation. See `vi/README.md`.
* `antora-playbook.yml` builds both into a single site.

Install the toolchain once, then build from the repository root.
...
The site is written to `build/site` and can be opened straight from the file system.
Pushing to `main` builds and publishes it to GitHub Pages through `.github/workflows/deploy-docs.yml`.
