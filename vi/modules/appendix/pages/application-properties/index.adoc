...
= Application Property thông dụng
...
Appendix này cung cấp danh sách các Spring Boot property thông dụng cùng tham chiếu tới các class bên dưới sử dụng chúng.

TIP: Spring Boot cung cấp nhiều cơ chế conversion với khả năng format value nâng cao.
Hãy xem xref:reference:features/external-config.adoc#features.external-config.typesafe-configuration-properties.conversion[phần conversion của property].

NOTE: Property có thể đến từ các jar file bổ sung trên classpath, nên bạn không nên coi đây là danh sách đầy đủ.
Ngoài ra, bạn có thể tự định nghĩa property của riêng mình.
